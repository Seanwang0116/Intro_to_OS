...
Thread::Thread(char *threadName, int threadID, int threadPriority)
{
    ...
    this_queue = new Manager(this, threadPriority);
...
// Todo ----
void Thread::setStatus(ThreadStatus ts, Thread *last){
    if(ts == READY) 
        this_queue->newToReady();
    else if(ts == RUNNING)
        this_queue->readyToRun(last);

    status = ts;
}

int Thread::compare_priority(Thread *t1, Thread *t2){
    return t2->getPriority() - t1->getPriority();
}

Thread::Manager::Manager(Thread * t, int priority):
    ref(t), priority(priority), tick(kernel->stats->totalTicks){}

void Thread::Manager::toReady() {
    tick = kernel->stats->totalTicks;
}
void Thread::Manager::readyToRun(Thread * last) {
    tick = kernel->stats->totalTicks;
    if (last) {
        ...
        DEBUG(dbgSch, "[C] Tick [" << kernel->stats->totalTicks 
            << "]: Thread [" << ref->ID 
            << "] is now selected for execution, thread [main] is replaced");
    }
}
